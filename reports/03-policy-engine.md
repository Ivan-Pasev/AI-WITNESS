...
A first temporary candidate, `db86aa611489599cab49a923ceef097479754e66`, was intentionally **not** published to `main` because GitHub Actions run `33669200931` failed TypeScript narrowing in `src/policy/evaluate.ts`. That candidate remains non-canonical evidence of the failed validation attempt. The final Phase-3 publication candidate was rebuilt directly from the Phase-2 base with the narrowing defect corrected and must pass the self-verification rule above before publication.
...
The public policy is versioned with exactly three top-level fields: `policyId`, `version`, and `rules`. Five rule classes are supported: `PATH_ALLOWLIST`, `PATH_DENYLIST`, `SECRET_SCAN`, `CHANGE_COUNT`, and `HUMAN_APPROVAL`.
...
Policy hashing reuses the Phase-2 accepted-value validation and canonicalization surface and freezes the distinct domain string `IRP1:POLICY`.
...
No real credentials, private keys, `.env` values, customer/personal data, private invention material, network calls, Hedera operations, or administrator override/bypass fields are introduced by Phase 3. Secret examples are explicitly synthetic markers. Automated Phase-7 repository-wide secret scanning has not yet run and is not claimed here.
