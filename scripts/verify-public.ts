...
async function localCase(slug: string): Promise<void> {
  ...
  console.log(JSON.stringify({ slug, ...verifyLocalBundle({ receipts }) }, null, 2));
}
...
async function main(): Promise<void> {
  ...
  if (mode === "network" || mode === "all") {
    ...
    console.log(
      JSON.stringify(
        { source: "hedera-testnet", ...(await verifyPhase5NetworkEvidence(evidence)) },
        null,
        2,
      ),
    );
